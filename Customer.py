...
class Customer:
    ...
    def insert_cust(self) -> str:
        ...
        val = (0, self.f_name, self.m_name, self.l_name, 
               self.username, self.__password, self.address, self.birth_date, 
               self.phone, self.email, self.age)
        
        my_cursor.execute(insert_sql, val)
        conn.commit()
        return ('Congratulations for Signing Up Successfully..')
